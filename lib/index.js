...
module.exports = function (babel) {
  ...
  function isolateExpression(expr, self, parent) {
  ...
  function childrenExpression(expr, children, parent) {
      ...
      enter: function(path, state) {
        ...
        path.traverse({
          JSXElement: {
            enter: function(path, state) {
              var expr = buildElementCall(path.get('openingElement'), state);
              var parent = state.opts.parent || 'parent';
              expr.arguments = expr.arguments.concat(childrenExpression(expr, path.node.children, parent), t.identifier(parent));
        ...
        path.replaceWith(isolateExpression(expr, self, parent));
      }
    }
  };
