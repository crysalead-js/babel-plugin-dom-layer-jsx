...
module.exports = function (babel) {
  ...
  function formatAttributes(attributes) {
    ...
    attributes.properties.forEach(function(attr) {
      ...
      if (name === "xmlns") {
        attrsPairs.push(t.objectProperty(t.identifier(name), attr.value)); //stringifiable
      } else if (name.substr(0, 6) === 'xmlns:') {
        name = name.substr(6);
        attrsNSPairs.push(t.objectProperty(t.identifier(name), attr.value)); //camelize
      } else if (name.substr(0, 3) === 'on-') {
        name = 'on' + name.substr(3);
        eventsPairs.push(t.objectProperty(t.identifier(name), attr.value)); //eventifiable
      } else if (name.substr(0, 5) === 'data-') {
        name = name.substr(5);
        dataPairs.push(t.objectProperty(t.identifier(name), attr.value)); //camelize
      } else {
        attrsPairs.push(t.objectProperty(t.identifier(name), attr.value)); //camelize
      }
    });
